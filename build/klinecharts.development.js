...
let yAxisRequireWidthSpace=this.yAxis.getRequiredWidthSpace();if(this.yAxis.yAxisPosition===YAxisPosition.LEFT){offsetLeft+=yAxisRequireWidthSpace;}else{offsetRight+=yAxisRequireWidthSpace;}}let requireXAxisHeight=this.xAxis.getRequiredHeightSpace();offsetBottom+=requireXAxisHeight;this.viewPortHandler.restrainViewPort(offsetLeft,offsetTop,offsetRight,offsetBottom);}/**
   ...
   * 设置数据
   * @param dataList
   */setDataList(dataList){if(dataList&&Array.isArray(dataList)){for(let i=0;i<dataList.length;i++){this.checkData(dataList[i]);}this.dataBounds.dataList=dataList;this.dataBounds.moveToLast();this.calcChartIndicator();this.freshen();}}/**
   * 添加数据
   * @param data
   * @param index
   */addData(data,index=this.dataBounds.dataList.length){this.checkData(data);this.dataBounds.dataList[index]=data;if(this.dataBounds.min+this.dataBounds.range>=this.dataBounds.dataList.length-1){this.dataBounds.moveToLast();}this.calcChartIndicator();this.freshen();}/**
   * 设置主指标类型
   * @param indicatorType
...
const klinecharts={init(dom){return new chart_KLineChart(dom);},version(){return "1.0.2";}};/* harmony default export */ var src_klinecharts = __webpack_exports__["default"] = (klinecharts);

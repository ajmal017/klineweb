...
this.mouseMode=CROSS;this.mouseDownPoint={x:0,y:0};}/**
   * 鼠标按下时事件
   * @param e
   */mouseDown(e){e.preventDefault();let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}this.mouseMode=DRAG;this.mouseDownPoint.x=e.x;this.mouseDownPoint.y=e.y;this.kline.tooltipChart.setCross(point.y,false);this.kline.freshen();}/**
   * 鼠标抬起时事件
   * @param e
   */mouseUp(e){e.preventDefault();let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}this.mouseMode=CROSS;this.kline.tooltipChart.setCross(point.y,true);this.kline.freshen();}mouseLeave(e){e.preventDefault();let point=this.getCanvasPoint(e);this.kline.tooltipChart.setCross(point.y,false);this.kline.freshen();}/**
   * 鼠标移动时事件
   * @param e
 ...
 * @param data
 * @returns {*}
 */function calculationMa(data){let ma5Num=0.0;let ma10Num=0.0;let ma20Num=0.0;let ma60Num=0.0;let ma5;let ma10;let ma20;let ma60;let totalTurnover=0.0;let totalVolume=0.0;for(let i=0;i<data.length;i++){checkData(data[i]);totalVolume+=data[i].volume;totalTurnover+=data[i].turnover||0;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}let close=data[i].close;ma5Num+=close;ma10Num+=close;ma20Num+=close;ma60Num+=close;if(i<5){ma5=ma5Num/(i+1);}else{ma5Num-=data[i-5].close;ma5=ma5Num/5;}if(i<10){ma10=ma10Num/(i+1);}else{ma10Num-=data[i-10].close;ma10=ma10Num/10;}if(i<20){ma20=ma20Num/(i+1);}else{ma20Num-=data[i-20].close;ma20=ma20Num/20;}if(i<60){ma60=ma60Num/(i+1);}else{ma60Num-=data[i-60].close;ma60=ma60Num/60;}data[i].ma={ma5,ma10,ma20,ma60};}return data;}/**
 * 计算成交量包含ma5、ma10、ma20
 *
 * @param data
 * @return
 */function calculationVol(data){let ma5s=0;let ma10s=0;let ma20s=0;let ma5;let ma10;let ma20;let totalTurnover=0;let totalVolume=0;for(let i=0;i<data.length;i++){checkData(data[i]);let turnover=data[i].turnover;totalVolume+=data[i].volume;totalTurnover+=turnover;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}let num=data[i].volume;ma5s+=num;ma10s+=num;ma20s+=num;if(i<5){ma5=ma5s/(i+1);}else{ma5s-=data[i-5].volume;ma5=ma5s/5;}if(i<10){ma10=ma10s/(i+1);}else{ma10s-=data[i-10].volume;ma10=ma10s/10;}if(i<20){ma20=ma20s/(i+1);}else{ma20s-=data[i-20].volume;ma20=ma20s/20;}data[i].vol={num,ma5,ma10,ma20};}return data;}/**
 * 计算MACD指标
 *
 * @param data
 * @return
...
let ema12;let ema26;let oldEma12=0;let oldEma26=0;let diff=0;let dea=0;let oldDea=0;let macd=0;let totalTurnover=0;let totalVolume=0;for(let i=0;i<data.length;i++){checkData(data[i]);let turnover=data[i].turnover;totalVolume+=data[i].volume;totalTurnover+=turnover;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}let closePrice=data[i].close;if(i>0){ema12=(2*closePrice+11*oldEma12)/13;ema26=(2*closePrice+25*oldEma26)/27;diff=ema12-ema26;dea=(diff*2+oldDea*8)/10;macd=(diff-dea)*2;oldEma12=ema12;oldEma26=ema26;oldDea=dea;}data[i].macd={diff,dea,macd};}return data;}/**
 * 计算BOLL指标
 *
 * @param data
 * @return
...
let ln;// n日内最高价
let hn;let totalTurnover=0;let totalVolume=0;for(let i=0;i<data.length;i++){checkData(data[i]);let turnover=data[i].turnover;totalVolume+=data[i].volume;totalTurnover+=turnover;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}// n日收盘价
...
let isIncreasing=false;let sar=0;let totalTurnover=0;let totalVolume=0;for(let i=0;i<data.length;i++){checkData(data[i]);let turnover=data[i].turnover;totalVolume+=data[i].volume;totalTurnover+=turnover;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}// 上一个周期的sar
 ...
 * 检查数据是否合法
 * @param data
 */function checkData(data){if(typeof data!=='object'||data.open===null||data.open===undefined||data.close===null||data.close===undefined||data.high===null||data.high===undefined||data.low===null||data.low===undefined||data.timestamp===null||data.timestamp===undefined||data.volume===null||data.volume===undefined||data.turnover===null||data.turnover===undefined){throw new Error('The data must be object and need to contain open, close, high, low, timestamp, volume, and turnover fields');}}

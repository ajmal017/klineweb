...
this.viewWidth=0;}/**
   ...
   * 获取中间点坐标
   */getContentCenter(){let point={};point.x=(this.contentRect.left+this.contentRect.right)/2;point.y=(this.contentRect.top+this.contentRect.bottom)/2;return point;}isInBoundsX(x){return this.isInBoundsLeft(x)&&this.isInBoundsRight(x);}isInBoundsLeft(x){return this.contentRect.left<=x+1;}isInBoundsRight(x){let tx=+(x*100).toFixed(0)/100;return this.contentRect.right>=tx-1;}}/* harmony default export */ var internal_ViewPortHandler = (ViewPortHandler);
...
this.currentDataPos=0;}/**
   ...
   * 计算当前数据的索引
   * @param x
   */calcCurrentDataIndex(x){let range=+Math.ceil((x-this.viewPortHandler.contentLeft())/this.dataSpace).toFixed(0);this.currentDataPos=this.min+range-1;}}/* harmony default export */ var internal_DataBounds = (DataBounds);
...
class Chart{constructor(dataBounds,viewPortHandler){this.dataBounds=dataBounds;this.viewPortHandler=viewPortHandler;this.chartTop=0;this.chartHeight=0;}/**
   ...
   * @param interval
   * @returns {number}
   */getY(yValue,yMin,interval){return+(this.chartTop+(1-(yValue-yMin)/interval)*this.chartHeight).toFixed(0);}}/* harmony default export */ var chart_Chart = (Chart);
// CONCATENATED MODULE: ./src/chart/GridChart.js
class GridChart_GridChart extends chart_Chart{constructor(grid,dataBounds,viewPortHandler){super(dataBounds,viewPortHandler);this.grid=grid;}draw(canvas){if(!this.grid.display){return;}canvas.strokeStyle=this.grid.lineColor;canvas.lineWidth=this.grid.lineSize;canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),this.viewPortHandler.contentTop()+0.5);canvas.lineTo(this.viewPortHandler.contentRight(),this.viewPortHandler.contentTop()+0.5);canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),this.viewPortHandler.contentBottom()+0.5);canvas.lineTo(this.viewPortHandler.contentRight(),this.viewPortHandler.contentBottom()+0.5);canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft()+0.5,this.viewPortHandler.contentTop());canvas.lineTo(this.viewPortHandler.contentLeft()+0.5,this.viewPortHandler.contentBottom());canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentRight()-0.5,this.viewPortHandler.contentTop());canvas.lineTo(this.viewPortHandler.contentRight()-0.5,this.viewPortHandler.contentBottom());canvas.stroke();canvas.closePath();}}/* harmony default export */ var chart_GridChart = (GridChart_GridChart);
 ...
 */};const YAxisTextPosition={/**
   ...
   * 获取y轴需要的宽度
   * @return number
   */getRequiredWidthSpace(){let width=0;if(this.tickText.position===YAxisTextPosition.OUTSIDE){width+=utils_utils.calcTextWidth(`${this.tickText.size*2}px Arial`,'0000000')+this.tickText.margin*2;if(this.display&&this.tickLine.display){width+=this.tickLine.size*2;}}if(this.display&&this.axisLine.display){width+=this.axisLine.size;}if(width===this.axisLine.size){return parseInt(width);}let maxWidth=width;if(maxWidth>0){maxWidth=this.yAxisMaxWidth*2;width=Math.min(maxWidth,Math.max(width,this.yAxisMinWidth*2));}return parseInt(width);}}/* harmony default export */ var component_YAxis = (YAxis_YAxis);
// CONCATENATED MODULE: ./src/chart/YAxisChart.js
class YAxisChart_YAxisChart extends chart_AxisChart{constructor(yAxis,dataBounds,viewPortHandler){super(dataBounds,viewPortHandler);this.axisMaximum=0;this.axisMinimum=0;this.axisRange=0;this.yAxis=yAxis;}drawAxisLine(canvas){if(!this.yAxis.display||!this.yAxis.axisLine.display){return;}canvas.strokeStyle=this.yAxis.axisLine.color||this.yAxis.color;canvas.lineWidth=this.yAxis.axisLine.size;canvas.beginPath();if(this.yAxis.yAxisPosition===YAxisPosition.LEFT){canvas.moveTo(this.viewPortHandler.contentLeft()+0.5,this.chartTop);canvas.lineTo(this.viewPortHandler.contentLeft()+0.5,this.chartTop+this.chartHeight);}else{canvas.moveTo(this.viewPortHandler.contentRight()+0.5,this.chartTop);canvas.lineTo(this.viewPortHandler.contentRight()+0.5,this.chartTop+this.chartHeight);}canvas.stroke();canvas.closePath();}/**
   * 绘制y轴上文字
   * @param canvas
   */drawAxisLabels(canvas){if(!this.yAxis.display||!this.yAxis.tickText.display){return;}let initX;if(this.yAxis.yAxisPosition===YAxisPosition.LEFT){if(this.yAxis.tickText.position===YAxisTextPosition.OUTSIDE){if(this.yAxis.display&&this.yAxis.tickLine.display){initX=this.viewPortHandler.contentLeft()-this.yAxis.tickLine.size*2-this.yAxis.tickText.margin*2;}else{initX=this.viewPortHandler.contentLeft()-this.yAxis.tickText.margin*2;}}else{if(this.yAxis.display&&this.yAxis.tickLine.display){initX=this.viewPortHandler.contentLeft()+this.yAxis.tickLine.size*2+this.yAxis.tickText.margin*2;}else{initX=this.viewPortHandler.contentLeft()+this.yAxis.tickText.margin*2;}}}else{if(this.yAxis.tickText.position===YAxisTextPosition.OUTSIDE){if(this.yAxis.display&&this.yAxis.tickLine.display){initX=this.viewPortHandler.contentRight()+this.yAxis.tickLine.size*2+this.yAxis.tickText.margin*2;}else{initX=this.viewPortHandler.contentRight()+this.yAxis.tickText.margin*2;}}else{if(this.yAxis.display&&this.yAxis.tickLine.display){initX=this.viewPortHandler.contentRight()-this.yAxis.tickLine.size*2-this.yAxis.tickText.margin*2;}else{initX=this.viewPortHandler.contentRight()-this.yAxis.tickText.margin*2;}}}canvas.textBaseline='middle';canvas.font=this.yAxis.tickText.size*2+'px Arial';canvas.fillStyle=this.yAxis.tickText.color||this.yAxis.color;let labelHeight=this.yAxis.tickText.size*2;let formatter=this.yAxis.tickText.valueFormatter;for(let i=0;i<this.values.length;i++){let labelY=this.getValueY(this.values[i]);let label=this.values[i].toString();if(formatter){label=formatter(this.values[i])||'--';}if(this.checkShowLabel(labelY,labelHeight)){if(this.yAxis.yAxisPosition===YAxisPosition.LEFT&&this.yAxis.tickText.position===YAxisTextPosition.OUTSIDE||this.yAxis.yAxisPosition===YAxisPosition.RIGHT&&this.yAxis.tickText.position!==YAxisTextPosition.OUTSIDE){canvas.textAlign='right';}else{canvas.textAlign='left';}canvas.fillText(label,initX,labelY);}}canvas.textAlign='left';}/**
   * 绘制y轴分割线
   * @param canvas
   */drawSeparatorLines(canvas){if(!this.yAxis.display||!this.yAxis.separatorLine.display){return;}canvas.strokeStyle=this.yAxis.separatorLine.color||this.yAxis.color;canvas.lineWidth=this.yAxis.separatorLine.size;let labelHeight=this.yAxis.tickText.size*2;if(this.yAxis.separatorLine.style===LineStyle.DASH){canvas.setLineDash(this.yAxis.separatorLine.dashValue);}for(let i=0;i<this.values.length;i++){let y=this.getValueY(this.values[i]);if(this.checkShowLabel(y,labelHeight)){canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),y+0.5);canvas.lineTo(this.viewPortHandler.contentRight(),y+0.5);canvas.stroke();canvas.closePath();}}canvas.setLineDash([]);}/**
   * 绘制刻度线
   * @param canvas
   */drawTickLines(canvas){if(!this.yAxis.display||!this.yAxis.tickLine.display){return;}canvas.lineWidth=1;canvas.strokeStyle=this.yAxis.axisLine.color||this.yAxis.color;let labelHeight=this.yAxis.tickText.size*2;let startX;let endX;if(this.yAxis.yAxisPosition===YAxisPosition.LEFT){startX=this.viewPortHandler.contentLeft();if(this.yAxis.tickText.position===YAxisTextPosition.OUTSIDE){endX=startX-this.yAxis.tickLine.size*2;}else{endX=startX+this.yAxis.tickLine.size*2;}}else{startX=this.viewPortHandler.contentRight();if(this.yAxis.tickText.position===YAxisTextPosition.OUTSIDE){endX=startX+this.yAxis.tickLine.size*2;}else{endX=startX-this.yAxis.tickLine.size*2;}}for(let i=0;i<this.values.length;i++){let y=this.getValueY(this.values[i]);if(this.checkShowLabel(y,labelHeight)){canvas.beginPath();canvas.moveTo(startX,y+0.5);canvas.lineTo(endX,y+0.5);canvas.stroke();canvas.closePath();}}}/**
...
class IndicatorChart_IndicatorChart extends chart_Chart{constructor(indicator,xAxis,yAxis,dataBounds,viewPortHandler,indicatorType=IndicatorType.MACD){super(dataBounds,viewPortHandler);this.indicator=indicator;this.xAxis=xAxis;this.yAxisChart=new chart_YAxisChart(yAxis,dataBounds,viewPortHandler);this.indicatorType=indicatorType;}setChartDimens(height,top){super.setChartDimens(height,top);this.yAxisChart.setChartDimens(height,top);}draw(canvas){if(this.isDisplayChart()){this.drawChartHorizontalSeparatorLine(canvas);this.yAxisChart.getYAxisDataMinMax(this.indicatorType);this.yAxisChart.drawSeparatorLines(canvas);this.yAxisChart.drawTickLines(canvas);this.drawIndicator(canvas);this.yAxisChart.drawAxisLine(canvas);this.yAxisChart.drawAxisLabels(canvas);}}/**
   ...
   * @param highY
   * @param lowY
   */drawOhlcLines(canvas,halfBarSpace,x,openY,closeY,highY,lowY){canvas.beginPath();canvas.moveTo(x,highY);canvas.lineTo(x,lowY);canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(x-halfBarSpace,openY+0.5);canvas.lineTo(x,openY+0.5);canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(x+halfBarSpace,closeY+0.5);canvas.lineTo(x,closeY+0.5);canvas.stroke();canvas.closePath();}/**
   * 获取y点坐标
   * @param yValue Float
...
this.lowestMarkData={};}/**
   ...
   * @param isHigh
   */drawLowestHighestPriceMark(canvas,x,price,color,textSize,valueFormatter,isHigh=false){canvas.save();canvas.beginPath();canvas.rect(this.viewPortHandler.contentLeft(),this.viewPortHandler.contentTop(),this.viewPortHandler.contentRight()-this.viewPortHandler.contentLeft(),this.chartTop+this.chartHeight);canvas.closePath();canvas.clip();let priceY=this.getValueY(price);let startX=x;let startY=priceY+(isHigh?-4:4)+0.5;canvas.textAlign='left';canvas.lineWidth=1;canvas.strokeStyle=color;canvas.fillStyle=color;canvas.beginPath();canvas.moveTo(startX+0.5,startY+0.5);canvas.lineTo(startX-4+0.5,startY+(isHigh?-4:4)+0.5);canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(startX+0.5,startY+0.5);canvas.lineTo(startX+4+0.5,startY+(isHigh?-4:4)+0.5);canvas.stroke();canvas.closePath();// 绘制竖线
canvas.beginPath();canvas.moveTo(startX+0.5,startY);startY=startY+(isHigh?-10:10);canvas.lineTo(startX+0.5,startY);canvas.stroke();canvas.closePath();canvas.beginPath();canvas.moveTo(startX,startY+0.5);canvas.lineTo(startX+10,startY+0.5);canvas.stroke();canvas.closePath();canvas.font=textSize*2+'px Arial';let value=price.toFixed(2);if(valueFormatter){value=valueFormatter(price)+'';}canvas.textBaseline='middle';canvas.fillText(value,startX+14,startY);canvas.restore();}/**
   * 绘制最新价标记
   * @param canvas
   */drawLastPriceMark(canvas){if(!this.candle.lastPriceMark.display||this.dataBounds.dataList.length===0){return;}let lastPrice=this.dataBounds.dataList[this.dataBounds.dataList.length-1].close;let priceY=this.getValueY(lastPrice);priceY=+Math.max(this.chartTop+this.chartHeight*0.05,Math.min(priceY,this.chartTop+this.chartHeight*0.98)).toFixed(0);canvas.strokeStyle=this.candle.lastPriceMark.lineColor;canvas.lineWidth=this.candle.lastPriceMark.lineSize;if(this.candle.lastPriceMark.lineStyle===LineStyle.DASH){canvas.setLineDash(this.candle.lastPriceMark.dashValue);}canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),priceY+0.5);canvas.lineTo(this.viewPortHandler.contentRight(),priceY+0.5);canvas.stroke();canvas.closePath();canvas.setLineDash([]);}/**
   * 绘制分时线
   * @param canvas
   */drawTimeLine(canvas){let timeLinePoints=[];let timeLineAreaPoints=[{x:this.viewPortHandler.contentLeft(),y:this.chartTop+this.chartHeight}];let averageLinePoints=[];canvas.lineWidth=this.candle.timeChart.timeLineSize;let kLineDataList=this.dataBounds.dataList;let startX=this.viewPortHandler.contentLeft();let i=this.dataBounds.min;let dataSpace=this.dataBounds.dataSpace-this.dataBounds.dataMarginSpaceRate*this.dataBounds.dataSpace;while(i<this.dataBounds.dataList.length&&i<this.dataBounds.min+this.dataBounds.range){let endX=startX+dataSpace;let x=+((startX+endX)/2).toFixed(0);let model=kLineDataList[i];let closeY=this.getValueY(model.close);let averagePriceY=this.getValueY(model.averagePrice);timeLinePoints.push({x:x,y:closeY});if(model.averagePrice){averageLinePoints.push({x:x,y:averagePriceY});}if(i===this.dataBounds.min){timeLineAreaPoints.push({x:this.viewPortHandler.contentLeft(),y:closeY});timeLineAreaPoints.push({x:x,y:closeY});}else if(i===this.dataBounds.min+this.dataBounds.range-1){timeLineAreaPoints.push({x:x,y:closeY});timeLineAreaPoints.push({x:this.viewPortHandler.contentRight(),y:closeY});timeLineAreaPoints.push({x:this.viewPortHandler.contentRight(),y:this.chartHeight+this.chartTop});}else if(i===this.dataBounds.dataList.length-1){timeLineAreaPoints.push({x:x,y:closeY});timeLineAreaPoints.push({x:x,y:this.chartTop+this.chartHeight});}else{timeLineAreaPoints.push({x:x,y:closeY});}startX+=this.dataBounds.dataSpace;++i;}if(timeLinePoints.length>0){// 绘制分时线
...
class XAxisChart_XAxisChart extends chart_AxisChart{constructor(xAxis,dataBounds,viewPortHandler){super(dataBounds,viewPortHandler);this.xAxis=xAxis;}/**
   ...
   * 绘制轴线
   * @param canvas
   */drawAxisLine(canvas){if(!this.xAxis.display||!this.xAxis.axisLine.display){return;}canvas.strokeStyle=this.xAxis.axisLine.color||this.xAxis.color;canvas.lineWidth=this.xAxis.axisLine.size;canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),this.viewPortHandler.contentBottom()+0.5);canvas.lineTo(this.viewPortHandler.contentRight(),this.viewPortHandler.contentBottom()+0.5);canvas.stroke();canvas.closePath();}/**
   * 绘制坐标轴上的文字
   * @param canvas Canvas
   */drawAxisLabels(canvas){if(!this.xAxis.display||!this.xAxis.tickText.display){return;}canvas.textBaseline='top';canvas.font=this.xAxis.tickText.size*2+'px Arial';canvas.textAlign='center';canvas.fillStyle=this.xAxis.tickText.color||this.xAxis.color;let positions=this.pointValuesToPixel();let startY=this.viewPortHandler.contentBottom()+this.xAxis.tickText.margin*2;if(this.display&&this.xAxis.tickLine.display){startY+=this.xAxis.tickLine.size*2;}let formatter=this.xAxis.tickText.valueFormatter;for(let i=0;i<positions.length;i+=2){let x=positions[i];if(this.viewPortHandler.isInBoundsX(x)){let kLineModel=this.dataBounds.dataList[parseInt(this.values[i/2])];let timestamp=kLineModel.timestamp;let label=utils_utils.formatDate(timestamp);if(formatter){label=formatter(kLineModel);}canvas.fillText(label,x,startY);}}}/**
   * 绘制分割线
   * @param canvas Canvas
   */drawSeparatorLines(canvas){if(!this.xAxis.display||!this.xAxis.separatorLine.display){return;}canvas.strokeStyle=this.xAxis.separatorLine.color||this.xAxis.color;canvas.lineWidth=this.xAxis.separatorLine.size;if(this.xAxis.separatorLine.style===LineStyle.DASH){canvas.setLineDash(this.xAxis.separatorLine.dashValue);}let positions=this.pointValuesToPixel();for(let i=0;i<positions.length;i+=2){let x=positions[i];if(this.viewPortHandler.isInBoundsX(x)){canvas.beginPath();canvas.moveTo(x+0.5,this.viewPortHandler.contentTop());canvas.lineTo(x+0.5,this.viewPortHandler.contentBottom());canvas.stroke();canvas.closePath();}}canvas.setLineDash([]);}/**
   * 绘制tick线
   * @param canvas Canvas
   */drawTickLines(canvas){if(!this.xAxis.display||!this.xAxis.tickLine.display){return;}canvas.lineWidth=1;canvas.strokeStyle=this.xAxis.axisLine.color||this.xAxis.color;let positions=this.pointValuesToPixel();let startY=this.viewPortHandler.contentBottom();let endY=startY+this.xAxis.tickLine.size*2;for(let i=0;i<positions.length;i+=2){let x=positions[i];if(this.viewPortHandler.isInBoundsX(x)){canvas.beginPath();canvas.moveTo(x+0.5,startY);canvas.lineTo(x+0.5,endY);canvas.stroke();canvas.closePath();}}}/**
   * 获取值对应的坐标点值
   * @return Array
   */pointValuesToPixel(){let positions=[];for(let i=0;i<this.valueCount*2;i+=2){let pos=this.values[i/2];positions[i]=parseInt((pos-this.dataBounds.min)*this.dataBounds.dataSpace+this.dataBounds.dataSpace*(1-this.dataBounds.dataMarginSpaceRate)/2);}return positions;}computeAxis(){let dataMin=this.dataBounds.min;let max=Math.min(dataMin+this.dataBounds.range-1,this.dataBounds.dataList.length-1);this.computeAxisValues(dataMin,max);}}/* harmony default export */ var chart_XAxisChart = (XAxisChart_XAxisChart);
 ...
 */const IndicatorDisplayRule={/**
   ...
   */NONE:'none'};class Tooltip_Tooltip{constructor(){/**
     ...
     */this.textSize=12;/**
     * 光标线配置
     */this.crossLine={display:true,style:LineStyle.SOLID,dashValue:[8,8],size:1,color:'#505050',text:{color:'#EDEDED',size:12,rectStrokeLineSize:1,rectStrokeLineColor:'#EDEDED',rectFillColor:'#505050',margin:4,valueFormatter:null}/**
     * 基础数据显示配置
...
this.yAxisLabelStrokePathPoints[0]={x:lineEndX,y:this.crossPoint.y};this.yAxisLabelStrokePathPoints[1]={x:lineEndX+crossTextMarginSpace,y:this.crossPoint.y-halfLabelHeight-crossTextMarginSpace};this.yAxisLabelStrokePathPoints[2]={x:lineEndX+crossTextMarginSpace*3+yAxisDataLabelWidth,y:this.yAxisLabelStrokePathPoints[1].y};this.yAxisLabelStrokePathPoints[3]={x:this.yAxisLabelStrokePathPoints[2].x,y:this.crossPoint.y+halfLabelHeight+crossTextMarginSpace};this.yAxisLabelStrokePathPoints[4]={x:this.yAxisLabelStrokePathPoints[1].x,y:this.yAxisLabelStrokePathPoints[3].y};}// 绘制十字光标水平线
canvas.lineWidth=this.tooltip.crossLine.size;canvas.strokeStyle=this.tooltip.crossLine.color;if(this.tooltip.crossLine.style===LineStyle.DASH){canvas.setLineDash(this.tooltip.crossLine.dashValue);}canvas.beginPath();canvas.moveTo(lineStartX,this.crossPoint.y+0.5);canvas.lineTo(lineEndX,this.crossPoint.y+0.5);canvas.stroke();canvas.closePath();canvas.setLineDash([]);// 绘制y轴文字外的边框
canvas.fillStyle=this.tooltip.crossLine.text.rectFillColor;canvas.beginPath();canvas.moveTo(this.yAxisLabelStrokePathPoints[0].x,this.yAxisLabelStrokePathPoints[0].y);for(let i=1;i<this.yAxisLabelStrokePathPoints.length;i++){canvas.lineTo(this.yAxisLabelStrokePathPoints[i].x,this.yAxisLabelStrokePathPoints[i].y);}canvas.closePath();canvas.fill();canvas.lineWidth=this.tooltip.crossLine.text.rectStrokeLineSize;canvas.strokeStyle=this.tooltip.crossLine.text.rectStrokeLineColor;canvas.beginPath();canvas.moveTo(this.yAxisLabelStrokePathPoints[0].x,this.yAxisLabelStrokePathPoints[0].y);for(let i=1;i<this.yAxisLabelStrokePathPoints.length;i++){canvas.lineTo(this.yAxisLabelStrokePathPoints[i].x,this.yAxisLabelStrokePathPoints[i].y);}canvas.closePath();canvas.stroke();canvas.textBaseline='middle';canvas.fillStyle=this.tooltip.crossLine.text.color;canvas.fillText(yAxisDataLabel,labelStartX,labelStartY);}/**
   * 获取十字光标y轴上的文字
...
// CONCATENATED MODULE: ./src/internal/MarkData.js
const MarkType={/**
   * 无
   */NONE:'none',/**
   * 水平直线
   ...
   */SEGMENT:'segment',/**
   * 箭头线
   */ARROW_LINE:'arrow_line'};const ActiveType={LINE:'line',POINT:'point',NONE:'none'};class MarkData{constructor(){this.markingType=MarkType.NONE;// 开始绘制时的点
this.startMarkPoint={x:0,y:0// 正在绘制的数据
...
this.arrowLineDatas=[];}}/* harmony default export */ var internal_MarkData = (MarkData);
// CONCATENATED MODULE: ./src/chart/MarkChart.js
class MarkChart_MarkChart extends chart_Chart{constructor(mark,markData,dataBounds,viewPortHandler){super(dataBounds,viewPortHandler);this.mark=mark;this.markData=markData;}draw(canvas){canvas.save();canvas.beginPath();canvas.rect(this.viewPortHandler.contentLeft(),this.chartTop,this.viewPortHandler.contentRight()-this.viewPortHandler.contentLeft(),this.chartHeight);canvas.closePath();canvas.clip();this.drawStartPoint(canvas);this.drawHorizontalVerticalLine(canvas,this.markData.horizontalLineDatas,MarkType.HORIZONTAL_LINE);this.drawHorizontalVerticalLine(canvas,this.markData.verticalLineDatas,MarkType.VERTICAL_LINE);this.drawLine(canvas);}/**
   * 绘制点
   * @param canvas
   * @param point
   * @param isHighlight
   */drawPoint(canvas,point,isHighlight){let radius=(isHighlight?this.mark.point.highlight.radius:this.mark.point.radius)*2;canvas.fillStyle=isHighlight?this.mark.point.highlight.fillColor:this.mark.point.fillColor;canvas.strokeStyle=isHighlight?this.mark.point.highlight.strokeColor:this.mark.point.strokeColor;canvas.lineWidth=1;canvas.beginPath();canvas.arc(point.x,point.y,radius,0,Math.PI*2,true);canvas.fill();canvas.closePath();canvas.beginPath();canvas.arc(point.x,point.y,radius,0,Math.PI*2,true);canvas.stroke();canvas.closePath();}/**
   * 绘制开始点
   * @param canvas
   */drawStartPoint(canvas){if(this.markData.markingType!==MarkType.NONE&&this.markData.markingDatas.length===0){let point=this.markData.startMarkPoint;this.drawPoint(canvas,point,true);}}/**
   * 绘制水平直线
   */drawHorizontalVerticalLine(canvas,lineDatas,markType){let lineDataLength=lineDatas.length;for(let i=0;i<lineDataLength;i++){let point=lineDatas[i];canvas.lineWidth=1;if(point.activeType===ActiveType.LINE||point.activeType===ActiveType.POINT){this.drawPoint(canvas,point,point.activeType===ActiveType.POINT);canvas.strokeStyle=this.mark.line.highlight.color;}canvas.beginPath();if(markType===MarkType.HORIZONTAL_LINE){canvas.moveTo(this.viewPortHandler.contentLeft(),point.y);canvas.lineTo(this.viewPortHandler.contentRight(),point.y);}else{canvas.moveTo(point.x,this.viewPortHandler.contentTop());canvas.lineTo(point.x,this.viewPortHandler.contentBottom());}canvas.stroke();canvas.closePath();}let point=this.markData.markingDatas[0];if(point&&this.markData.markingType===markType){canvas.lineWidth=1;canvas.strokeStyle=this.mark.line.highlight.color;canvas.beginPath();if(markType===MarkType.HORIZONTAL_LINE){canvas.moveTo(this.viewPortHandler.contentLeft(),point.y);canvas.lineTo(this.viewPortHandler.contentRight(),point.y);}else{canvas.moveTo(point.x,this.viewPortHandler.contentTop());canvas.lineTo(point.x,this.viewPortHandler.contentBottom());}canvas.stroke();canvas.closePath();this.drawPoint(canvas,point,true);}}/**
   * 绘制直线
   * @param canvas
   */drawLine(canvas){let lineDataLength=this.markData.lineDatas.length;canvas.lineWidth=1;canvas.strokeStyle='#ffffff';for(let i=0;i<lineDataLength;i++){let lineData=this.markData.lineDatas[i];let points=lineData.points;canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),this.getY(this.viewPortHandler.contentLeft(),points[0],points[1]));canvas.lineTo(this.viewPortHandler.contentRight(),this.getY(this.viewPortHandler.contentRight(),points[0],points[1]));canvas.stroke();canvas.closePath();}let markingPointSize=this.markData.markingDatas.length;if(markingPointSize>0&&this.markData.markingType===MarkType.LINE){if(markingPointSize===1){this.drawPoint(canvas,this.markData.markingDatas[0],true);}else{this.drawPoint(canvas,this.markData.markingDatas[0],false);this.drawPoint(canvas,this.markData.markingDatas[1],true);canvas.beginPath();canvas.moveTo(this.viewPortHandler.contentLeft(),this.getY(this.viewPortHandler.contentLeft(),this.markData.markingDatas[0],this.markData.markingDatas[1]));canvas.lineTo(this.viewPortHandler.contentRight(),this.getY(this.viewPortHandler.contentRight(),this.markData.markingDatas[0],this.markData.markingDatas[1]));canvas.stroke();canvas.closePath();}}}/**
   * 获取x点对应的y轴坐标
   * @param x
   * @param point1
   * @param point2
   * @returns {number}
   */getY(x,point1,point2){return(point2.y-point1.y)/(point2.x-point1.x)*x+point1.y-(point2.y-point1.y)/(point2.x-point1.x)*point1.x;}}/* harmony default export */ var chart_MarkChart = (MarkChart_MarkChart);
...
class XAxis_XAxis extends component_Axis{constructor(){super();/**
   ...
   * 计算x轴需要的高度
   * @return number
   */getRequiredHeightSpace(){let height=this.tickText.size*2+this.tickText.margin*2;if(this.display&&this.tickLine.display){height+=this.tickLine.size*2;}if(this.display&&this.axisLine.display){height+=this.axisLine.size;}let maxHeight=height;if(this.xAxisMaxHeight>0){maxHeight=this.xAxisMaxHeight*2;}height=Math.max(this.xAxisMinHeight*2,Math.min(height,maxHeight));return parseInt(height);}}/* harmony default export */ var component_XAxis = (XAxis_XAxis);
...
// CONCATENATED MODULE: ./src/component/Mark.js
class DrawLine{constructor(){this.line={color:'#DEDEDE',highlight:{color:'#ffffff'}};this.point={fillColor:'#DEDEDE',strokeColor:'#DEDEDE',radius:4,highlight:{fillColor:'#FFFFFF',strokeColor:'#FFFFFF',radius:5}};}}/* harmony default export */ var Mark = (DrawLine);
...
class Event{constructor(kline,dataBounds,viewPortHandler){this.kline=kline;this.dataBounds=dataBounds;this.viewPortHandler=viewPortHandler;}/**
   ...
   * @param point
   * @returns {boolean}
   */isValidEvent(point){return!(point.x<this.viewPortHandler.contentLeft()||point.x>this.viewPortHandler.contentRight()||point.y<this.viewPortHandler.contentTop()||point.y>this.viewPortHandler.contentBottom());}/**
   * 获取事件对应画布上的点
   * @param e
   * @returns {{x: number, y: number}}
   */getCanvasPoint(e){let rect=this.kline.markCanvasDom.getBoundingClientRect();let x=Math.round(e.clientX-rect.left);let y=Math.round(e.clientY-rect.top);return{x:x*2,y:y*2};}/**
   * 阻止事件
   * @param e
   */stopEvent(e){if(e&&e.stopPropagation){e.stopPropagation();}else{window.event.cancelBubble=true;}if(e&&e.preventDefault){e.preventDefault();}else{window.event.returnValue=false;}}}/* harmony default export */ var event_Event = (Event);
...
this.mouseMode=CROSS;this.mouseDownPoint={x:0,y:0};}/**
   * 鼠标按下时事件
   * @param e
   */mouseDown(e){this.stopEvent(e);if(e.button===0){let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}this.mouseMode=DRAG;this.mouseDownPoint.x=e.x;this.mouseDownPoint.y=e.y;this.kline.tooltipChart.setCross(point.y,false);this.kline.freshen(FRESHEN_TOOLTIP);}}/**
   * 鼠标抬起时事件
   * @param e
   */mouseUp(e){this.stopEvent(e);let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}this.mouseMode=CROSS;this.kline.tooltipChart.setCross(point.y,true);this.kline.freshen(FRESHEN_TOOLTIP);}mouseLeave(e){this.stopEvent(e);let point=this.getCanvasPoint(e);this.kline.tooltipChart.setCross(point.y,false);this.kline.freshen(FRESHEN_TOOLTIP);}/**
   * 鼠标移动时事件
   * @param e
   */mouseMove(e){this.stopEvent(e);let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){this.kline.tooltipChart.setCross(point.y,false);this.kline.freshen(FRESHEN_TOOLTIP);return;}if(this.mouseMode===DRAG){let moveDist=e.x-this.mouseDownPoint.x;if(moveDist>this.dataBounds.dataSpace/2){if(this.dataBounds.min===0||this.dataBounds.dataList.length<this.dataBounds.range){return false;}this.mouseDownPoint.x=e.x;let moveRange=+Math.abs(moveDist/this.dataBounds.dataSpace).toFixed(0);if(moveRange===0){moveRange=1;}this.dataBounds.min-=moveRange;if(this.dataBounds.min<=0){this.dataBounds.min=0;}this.kline.freshen(FRESHEN_CHART);}else if(moveDist<0-this.dataBounds.dataSpace/2){if(this.dataBounds.min+this.dataBounds.range===this.dataBounds.dataList.length||this.dataBounds.dataList.length<this.dataBounds.range){return false;}this.mouseDownPoint.x=e.x;let moveRange=+Math.abs(moveDist/this.dataBounds.dataSpace).toFixed(0);if(moveRange===0){moveRange=1;}this.dataBounds.min+=moveRange;if(this.dataBounds.min>=this.dataBounds.dataList.length-this.dataBounds.range){this.dataBounds.min=this.dataBounds.dataList.length-this.dataBounds.range;}this.kline.freshen();}}else if(this.mouseMode===CROSS){this.dataBounds.calcCurrentDataIndex(point.x);this.kline.tooltipChart.setCross(point.y,true);this.kline.freshen(FRESHEN_TOOLTIP);}}/**
   * 鼠标滚轮事件
   * @param e
   */mouseWheel(e){let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}this.stopEvent(e);let touchStartPosition=this.dataBounds.min;let touchRange=this.dataBounds.range;let delta=Math.max(-1,Math.min(1,e.wheelDelta||-e.deltaY));// 是否缩小
...
this.dataBounds.range=+(touchRange/scaleX).toFixed(0);this.dataBounds.range=Math.min(Math.max(this.dataBounds.range,this.dataBounds.minRange),this.dataBounds.maxRange);this.dataBounds.min=touchStartPosition+touchRange-this.dataBounds.range;if(this.dataBounds.min+this.dataBounds.range>this.dataBounds.dataList.length){this.dataBounds.min=0;}if(this.dataBounds.min<0){this.dataBounds.min=0;}this.kline.freshen(FRESHEN_CHART);}}/* harmony default export */ var event_MouseEvent = (MouseEvent_MouseEvent);
...
this.touchMode=TOUCH_NO;this.touchStartPoint={x:0,y:0};this.touchMovePoint={x:0,y:0};this.touchCrossPoint={x:0,y:0};this.savedDist=1;this.savedXDist=1;this.touchRange=this.dataBounds.range;this.touchStartPosition=this.dataBounds.min;this.delayTimeout=null;this.delayActiveCross=()=>{if(this.touchMode===TOUCH_NO||this.touchMode===TOUCH_CROSS_CANCEL){if(this.kline){this.touchMode=TOUCH_CROSS;this.touchCrossPoint={x:this.touchStartPoint.x,y:this.touchStartPoint.y};dataBounds.calcCurrentDataIndex(this.touchCrossPoint.x);this.kline.tooltipChart.setCross(this.touchCrossPoint.y,true);this.kline.freshen(FRESHEN_TOOLTIP);}}};}/**
   * 触摸事件开始
   * @param e
   */touchStart(e){if(e.targetTouches.length===1){let point=this.getCanvasPoint(e.targetTouches[0]);this.touchStartPoint={x:point.x,y:point.y};this.touchMovePoint={x:point.x,y:point.y};if(!this.isValidEvent(this.touchStartPoint)){return;}if(this.touchMode===TOUCH_CROSS){this.stopEvent(e);let crossRadius=this.distance(point.x,this.touchCrossPoint.x,point.y,this.touchCrossPoint.y);if(crossRadius<10){this.performCross(e);}else{this.touchMode=TOUCH_CROSS_CANCEL;this.kline.tooltipChart.setCross(0,false);this.kline.freshen(FRESHEN_TOOLTIP);}}else{this.touchMode=TOUCH_NO;}this.removeDelayActiveCross();this.postDelayDelayActiveCross();}else if(e.targetTouches.length>1){if(!this.isValidEvent(this.touchStartPoint)){return;}if(this.touchMode!==TOUCH_CROSS){this.stopEvent(e);this.savedDist=this.spacing(e);this.savedXDist=this.getXDist(e);if(this.savedDist>3){this.touchMode=TOUCH_ZOOM;}this.touchRange=this.dataBounds.range;this.touchStartPosition=this.dataBounds.min;}}}/**
   * 触摸事件移动
   * @param e
   */touchMove(e){if(!this.isValidEvent(this.touchStartPoint)){return;}switch(this.touchMode){case TOUCH_ZOOM:{this.stopEvent(e);this.performZoom(e);break;}case TOUCH_DRAG:{this.stopEvent(e);this.performDrag(e);break;}case TOUCH_CROSS:{this.stopEvent(e);this.performCross(e);break;}case TOUCH_CROSS_CANCEL:{this.removeDelayActiveCross();break;}case TOUCH_NO:{let point=this.getCanvasPoint(e.targetTouches[0]);let distance=Math.abs(this.distance(point.x,this.touchStartPoint.x,point.y,this.touchStartPoint.y));if(distance>10){let distanceX=Math.abs(point.x-this.touchStartPoint.x);let distanceY=Math.abs(point.y-this.touchStartPoint.y);if(distanceY<=distanceX){this.stopEvent(e);this.kline.tooltipChart.setCross(0,false);this.touchMode=TOUCH_DRAG;this.kline.freshen(FRESHEN_TOOLTIP);}}this.removeDelayActiveCross();}}}/**
   * 触摸事件结束
   * @param e
   */touchEnd(e){if(!this.isValidEvent(this.touchStartPoint)){return;}this.stopEvent(e);if(e.targetTouches.length>0){if(this.touchMode===TOUCH_CROSS){this.performCross(e);}else{this.touchMode=TOUCH_POST_ZOOM;}}else{this.removeDelayActiveCross();if(this.touchMode!==TOUCH_CROSS){// 拿起
this.touchMode=TOUCH_NO;this.kline.tooltipChart.setCross(0,false);this.kline.freshen(FRESHEN_TOOLTIP);}}}/**
   * 处理拖拽视图事件
   * @param e
   * @returns {boolean}
   */performDrag(e){// 左右滑动事件
let point=this.getCanvasPoint(e.targetTouches[0]);let moveDist=point.x-this.touchMovePoint.x;if(moveDist<0-this.dataBounds.dataSpace/2){if(this.dataBounds.min+this.dataBounds.range===this.dataBounds.dataList.length||this.dataBounds.dataList.length<this.dataBounds.range){return false;}this.touchMovePoint.x=point.x;let moveRange=+Math.abs(moveDist/this.dataBounds.dataSpace).toFixed(0);if(moveRange===0){moveRange=1;}this.dataBounds.min+=moveRange;if(this.dataBounds.min>=this.dataBounds.dataList.length-this.dataBounds.range){this.dataBounds.min=this.dataBounds.dataList.length-this.dataBounds.range;}this.kline.freshen(FRESHEN_CHART);}else if(moveDist>this.dataBounds.dataSpace/2){if(this.dataBounds.min===0||this.dataBounds.dataList.length<this.dataBounds.range){return false;}this.touchMovePoint.x=point.x;let moveRange=+Math.abs(moveDist/this.dataBounds.dataSpace).toFixed(0);if(moveRange===0){moveRange=1;}this.dataBounds.min-=moveRange;if(this.dataBounds.min<=0){this.dataBounds.min=0;}this.kline.freshen(FRESHEN_CHART);}}/**
   * 处理缩放
   * @param e
   * @returns {boolean}
...
return false;}}// 计算缩放后的range大小
this.dataBounds.range=+(this.touchRange/scaleX).toFixed(0);this.dataBounds.range=Math.min(Math.max(this.dataBounds.range,this.dataBounds.minRange),this.dataBounds.maxRange);this.dataBounds.min=this.touchStartPosition+this.touchRange-this.dataBounds.range;if(this.dataBounds.min+this.dataBounds.range>this.dataBounds.dataList.length){this.dataBounds.min=0;}if(this.dataBounds.min<0){this.dataBounds.min=0;}this.kline.freshen(FRESHEN_CHART);}}}/**
   * 处理移动光标
   * @param e
   * @returns {boolean}
   */performCross(e){let point=this.getCanvasPoint(e.targetTouches[0]);this.touchCrossPoint={x:point.x,y:point.y};this.dataBounds.calcCurrentDataIndex(this.touchCrossPoint.x);this.kline.tooltipChart.setCross(this.touchCrossPoint.y,true);this.kline.freshen(FRESHEN_TOOLTIP);}/**
   * 执行延迟事件
   ...
   * @param e
   * @returns {*}
   */spacing(e){if(e.targetTouches.length<2){return 0;}let point1=this.getCanvasPoint(e.targetTouches[0]);let point2=this.getCanvasPoint(e.targetTouches[1]);let x=Math.abs(point1.x-point2.x);let y=Math.abs(point1.y-point2.y);return Math.sqrt(x*x+y*y);}/**
   * 获取两点间x的距离
   * @param e
   * @returns {number}
   */getXDist(e){let point1=this.getCanvasPoint(e.targetTouches[0]);let point2=this.getCanvasPoint(e.targetTouches[1]);return Math.abs(point1.x-point2.y);}}/* harmony default export */ var event_TouchEvent = (TouchEvent_TouchEvent);
// CONCATENATED MODULE: ./src/internal/event/MarkEvent.js
class MarkEvent_MarkEvent extends event_Event{constructor(kline,markData,dataBounds,viewPortHandler){super(kline,dataBounds,viewPortHandler);this.markData=markData;}/**
   * 绘制结束
   */drawLineEnd(){this.markData.markingType=MarkType.NONE;this.markData.markingDatas=[];this.kline.tooltipCanvasDom.style.display='';}/**
   * 开始绘制
   */drawLineStart(markingType){this.markData.markingType=markingType;this.markData.markingDatas=[];this.kline.tooltipCanvasDom.style.display='none';}/**
   * 鼠标按下时事件
   * @param e
   */mouseDown(e){this.stopEvent(e);let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}if(e.button===0){if(this.markData.markingType===MarkType.NONE){if(this.performHorizontalLineMouseLeftClick(point)||this.performVerticalLineMouseLeftClick(point)){this.kline.freshen(FRESHEN_DRAW_LINE_CHART);return;}return;}if(this.markData.markingDatas.length===0){this.markData.markingDatas[0]=point;this.kline.freshen(FRESHEN_DRAW_LINE_CHART);}else if(this.markData.markingDatas.length===1){if(this.markData.markingType===MarkType.HORIZONTAL_LINE||this.markData.markingType===MarkType.VERTICAL_LINE){if(this.markData.markingType===MarkType.HORIZONTAL_LINE){this.markData.horizontalLineDatas.push(point);}else{this.markData.verticalLineDatas.push(point);}this.drawLineEnd();}this.kline.freshen(FRESHEN_DRAW_LINE_CHART);}else if(this.markData.markingDatas.length===2){this.markData.lineDatas.push({points:[this.markData.markingDatas[0],point]});this.drawLineEnd();this.kline.freshen(FRESHEN_DRAW_LINE_CHART);}}else if(e.button===2){if(this.markData.markingType!==MarkType.NONE){this.drawLineEnd();this.kline.freshen(FRESHEN_DRAW_LINE_CHART);return;}if(this.performHorizontalLineMouseRightClick(point)||this.performVerticalLineMouseRightClick(point)){this.kline.freshen(FRESHEN_DRAW_LINE_CHART);// return
}}}/**
   * 处理水平直线鼠标左键点击事件
   * @param point
   */performHorizontalLineMouseLeftClick(point){let horizontalLineDataLength=this.markData.horizontalLineDatas.length;for(let i=0;i<horizontalLineDataLength;i++){let lineData=this.markData.horizontalLineDatas[i];if(point.y<lineData.y+10&&point.y>lineData.y-10&&point.x<lineData.x+10&&point.x>lineData.x-10){this.markData.horizontalLineDatas.splice(i,1);this.drawLineStart(MarkType.HORIZONTAL_LINE);this.markData.markingDatas[0]=point;return true;}}return false;}/**
   * 处理水平直线鼠标左键点击事件
   * @param point
   */performHorizontalLineMouseRightClick(point){let horizontalLineDataLength=this.markData.horizontalLineDatas.length;for(let i=0;i<horizontalLineDataLength;i++){let lineData=this.markData.horizontalLineDatas[i];if(point.x<lineData.x+10&&point.x>lineData.x-10){if(point.y<lineData.y+10&&point.y>lineData.y-10){if(this.markData.markingType===MarkType.NONE){this.markData.horizontalLineDatas.splice(i,1);this.drawLineEnd();return true;}}}else{if(point.y<lineData.y+4&&point.y>lineData.y-4){if(this.markData.markingType===MarkType.NONE){this.markData.horizontalLineDatas.splice(i,1);this.drawLineEnd();return true;}}}}return false;}/**
   * 处理垂直直线鼠标左键点击事件
   * @param point
   */performVerticalLineMouseLeftClick(point){let verticalLineDataLength=this.markData.verticalLineDatas.length;for(let i=0;i<verticalLineDataLength;i++){let lineData=this.markData.verticalLineDatas[i];if(point.y<lineData.y+10&&point.y>lineData.y-10&&point.x<lineData.x+10&&point.x>lineData.x-10){this.markData.verticalLineDatas.splice(i,1);this.drawLineStart(MarkType.VERTICAL_LINE);this.markData.markingDatas[0]=point;return true;}}return false;}/**
   * 处理水平直线鼠标左键点击事件
   * @param point
   */performVerticalLineMouseRightClick(point){let verticalLineDataLength=this.markData.verticalLineDatas.length;for(let i=0;i<verticalLineDataLength;i++){let lineData=this.markData.verticalLineDatas[i];if(point.y<lineData.y+10&&point.y>lineData.y-10){if(point.y<lineData.x+10&&point.x>lineData.x-10){if(this.markData.markingType===MarkType.NONE){this.markData.verticalLineDatas.splice(i,1);this.drawLineEnd();return true;}}}else{if(point.x<lineData.x+4&&point.x>lineData.x-4){if(this.markData.markingType===MarkType.NONE){this.markData.verticalLineDatas.splice(i,1);this.drawLineEnd();return true;}}}}return false;}/**
   * 鼠标抬起时事件
   * @param e
   */mouseUp(e){this.stopEvent(e);}mouseLeave(e){this.stopEvent(e);}/**
   * 鼠标移动时事件
   * @param e
   */mouseMove(e){this.stopEvent(e);let point=this.getCanvasPoint(e);if(!this.isValidEvent(point)){return;}if(this.markData.markingType===MarkType.NONE){if(this.performHorizontalLineEventMove(point)||this.performVerticalLineEventMove(point)){this.kline.freshen(FRESHEN_DRAW_LINE_CHART);return;}}if(this.markData.markingDatas.length===0){this.markData.startMarkPoint=point;this.kline.freshen(FRESHEN_DRAW_LINE_CHART);}else if(this.markData.markingDatas.length===1){if(this.markData.markingType===MarkType.HORIZONTAL_LINE||this.markData.markingType===MarkType.VERTICAL_LINE){this.markData.markingDatas[0]=point;}else{this.markData.markingDatas[1]=point;}this.kline.freshen(FRESHEN_DRAW_LINE_CHART);}else if(this.markData.markingDatas.length===2){this.markData.markingDatas[1]=point;this.kline.freshen(FRESHEN_DRAW_LINE_CHART);}}/**
   * 处理鼠标移动时对水平直线的处理
   * @param point
   */performHorizontalLineEventMove(point){let horizontalLineDataLength=this.markData.horizontalLineDatas.length;let isActive=false;for(let i=0;i<horizontalLineDataLength;i++){let lineData=this.markData.horizontalLineDatas[i];if(point.x<lineData.x+10&&point.x>lineData.x-10){if(point.y<lineData.y+10&&point.y>lineData.y-10){this.markData.horizontalLineDatas[i].activeType=ActiveType.POINT;isActive=true;}}else{if(point.y<lineData.y+4&&point.y>lineData.y-4){this.markData.horizontalLineDatas[i].activeType=ActiveType.LINE;isActive=true;}else{this.markData.horizontalLineDatas[i].activeType=ActiveType.NONE;}}}return isActive;}/**
   * 处理鼠标移动时对水平直线的处理
   * @param point
   */performVerticalLineEventMove(point){let verticalLineDataLength=this.markData.verticalLineDatas.length;let isActive=false;for(let i=0;i<verticalLineDataLength;i++){let lineData=this.markData.verticalLineDatas[i];if(point.y<lineData.y+10&&point.y>lineData.y-10){if(point.x<lineData.x+10&&point.x>lineData.x-10){this.markData.verticalLineDatas[i].activeType=ActiveType.POINT;isActive=true;}}else{if(point.x<lineData.x+4&&point.x>lineData.x-4){this.markData.verticalLineDatas[i].activeType=ActiveType.LINE;isActive=true;}else{this.markData.verticalLineDatas[i].activeType=ActiveType.NONE;}}}return isActive;}/**
   * 鼠标滚轮事件
   * @param e
   */mouseWheel(e){this.stopEvent(e);}/**
   * 是否是有效事件
   * @param point
   * @returns {boolean}
   */isValidEvent(point){return!(point.x<this.viewPortHandler.contentLeft()||point.x>this.viewPortHandler.contentRight()||point.y<this.viewPortHandler.contentTop()||point.y>this.kline.candleChart.chartTop+this.kline.candleChart.chartHeight);}}/* harmony default export */ var event_MarkEvent = (MarkEvent_MarkEvent);
// CONCATENATED MODULE: ./src/utils/indicatorCalculation.js
/**
 * 计算均线数据
 * @param data
 * @returns {*}
 */async function calculationMa(data){let ma5Num=0.0;let ma10Num=0.0;let ma20Num=0.0;let ma60Num=0.0;let ma5;let ma10;let ma20;let ma60;let totalTurnover=0.0;let totalVolume=0.0;for(let i=0;i<data.length;i++){totalVolume+=data[i].volume;totalTurnover+=data[i].turnover||0;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}let close=data[i].close;ma5Num+=close;ma10Num+=close;ma20Num+=close;ma60Num+=close;if(i<5){ma5=ma5Num/(i+1);}else{ma5Num-=data[i-5].close;ma5=ma5Num/5;}if(i<10){ma10=ma10Num/(i+1);}else{ma10Num-=data[i-10].close;ma10=ma10Num/10;}if(i<20){ma20=ma20Num/(i+1);}else{ma20Num-=data[i-20].close;ma20=ma20Num/20;}if(i<60){ma60=ma60Num/(i+1);}else{ma60Num-=data[i-60].close;ma60=ma60Num/60;}data[i].ma={ma5,ma10,ma20,ma60};}return data;}/**
 * 计算成交量包含ma5、ma10、ma20
 *
 * @param data
 * @return
 */async function calculationVol(data){let ma5s=0;let ma10s=0;let ma20s=0;let ma5;let ma10;let ma20;let totalTurnover=0;let totalVolume=0;for(let i=0;i<data.length;i++){let turnover=data[i].turnover;totalVolume+=data[i].volume;totalTurnover+=turnover;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}let num=data[i].volume;ma5s+=num;ma10s+=num;ma20s+=num;if(i<5){ma5=ma5s/(i+1);}else{ma5s-=data[i-5].volume;ma5=ma5s/5;}if(i<10){ma10=ma10s/(i+1);}else{ma10s-=data[i-10].volume;ma10=ma10s/10;}if(i<20){ma20=ma20s/(i+1);}else{ma20s-=data[i-20].volume;ma20=ma20s/20;}data[i].vol={num,ma5,ma10,ma20};}return data;}/**
 * 计算MACD指标
 *
 * @param data
 * @return
 */async function calculationMacd(data){// MACD：参数快线移动平均、慢线移动平均、移动平均，
// 参数值12、26、9。
 ...
 * @param data
 * @return
 */async function calculationBoll(data){let closes26=0;// MA
 ...
 * @param data
 * @return
 */async function calculationKdj(data){let k;let d;let j;// n日内最低价
let ln;// n日内最高价
 ...
 * @param data
 * @return
 */async function calculationRsi(data){// N日RSI =
 ...
 * @param data
 * @return
 */async function calculationBias(data){// 乖离率=[(当日收盘价-N日平均价)/N日平均价]*100%
// 参数：6，12、24
 ...
 * @param data
 * @return
 */async function calculationBrar(data){// 参数是26。
 ...
 * @param data
 * @return
 */async function calculationCci(data){// 中价与 中价的N日内移动平均 的差 除以 N日内中价的平均绝对偏差
 ...
 * @param data
 * @return
 */async function calculationDmi(data){// 参数 14，6
 ...
 * @param data
 * @return
 */async function calculationObv(data){let sumObv=0;let maObv;let totalTurnover=0;let totalVolume=0;for(let i=0;i<data.length;i++){let turnover=data[i].turnover;totalVolume+=data[i].volume;totalTurnover+=turnover;if(totalVolume!==0){data[i].averagePrice=totalTurnover/totalVolume;}let closePrice=data[i].close;let highestPrice=data[i].high;let highSubClose=highestPrice-closePrice;let obv=data[i].volume*(closePrice-data[i].low-highSubClose)/highestPrice;sumObv+=obv;if(i<30){maObv=sumObv/(i+1);}else{sumObv=sumObv-data[i-30].obv.obv;maObv=sumObv/30;}data[i].obv={obv,maObv};}return data;}/**
 * 计算vr指标
 * 默认参数24 ， 30
...
const FRESHEN_ALL='all';const FRESHEN_TOOLTIP='tooltip';const FRESHEN_CHART='chart';const FRESHEN_CANDLE_CHART='candle_chart';const FRESHEN_VOL_CHART='vol_chart';const FRESHEN_INDICATOR_CHART='indicator_chart';const FRESHEN_DRAW_LINE_CHART='draw_line_chart';class KLineChart_KLineChart{constructor(dom){this.rootDom=dom;this.chartCanvasDom=null;this.chartCanvas=null;this.tooltipCanvasDom=null;this.tooltipCanvas=null;this.markCanvasDom=null;this.markCanvas=null;this.viewPortHandler=new internal_ViewPortHandler();this.dataBounds=new internal_DataBounds(this.viewPortHandler);this.grid=new component_Grid();this.yAxis=new component_YAxis();this.xAxis=new component_XAxis();this.candle=new component_Candle();this.indicator=new component_Indicator();this.tooltip=new component_Tooltip();this.gridChart=new chart_GridChart(this.grid,this.dataBounds,this.viewPortHandler);this.candleChart=new chart_CandleChart(this.candle,this.indicator,this.yAxis,this.dataBounds,this.viewPortHandler);this.volChart=new chart_IndicatorChart(this.indicator,this.xAxis,this.yAxis,this.dataBounds,this.viewPortHandler,IndicatorType.VOL);this.indicatorChart=new chart_IndicatorChart(this.indicator,this.xAxis,this.yAxis,this.dataBounds,this.viewPortHandler);this.xAxisChart=new chart_XAxisChart(this.xAxis,this.dataBounds,this.viewPortHandler);this.tooltipChart=new chart_TooltipChart(this.tooltip,this.candle,this.indicator,this.yAxis,this.candleChart,this.volChart,this.indicatorChart,this.dataBounds,this.viewPortHandler);this.mark=new Mark();this.markData=new internal_MarkData();this.markChart=new chart_MarkChart(this.mark,this.markData,this.dataBounds,this.viewPortHandler);// 是否需要计算整个绘图区域的尺寸
this.isShouldCalcOffset=true;// 是否需要计算图的高度
this.isShouldCalcChartHeight=true;this.init();}/**
   * 初始化
   */init(){this.rootDom.style.position='relative';this.chartCanvasDom=document.createElement('canvas');this.initCanvas(this.chartCanvasDom);this.chartCanvas=this.chartCanvasDom.getContext('2d');this.markCanvasDom=document.createElement('canvas');this.initCanvas(this.markCanvasDom);this.markCanvas=this.markCanvasDom.getContext('2d');this.tooltipCanvasDom=document.createElement('canvas');this.initCanvas(this.tooltipCanvasDom);this.tooltipCanvas=this.tooltipCanvasDom.getContext('2d');this.initEvent();this.resize();}/**
   * 初始化画布
   */initCanvas(canvasDom){canvasDom.style.position='absolute';canvasDom.style.top='0';canvasDom.style.right='0';canvasDom.style.bottom='0';canvasDom.style.left='0';this.rootDom.appendChild(canvasDom);}/**
   * 初始化事件
   */initEvent(){try{let isMobile=/Android|webOS|iPhone|iPod|BlackBerry|UCBrowser/i.test(navigator.userAgent);if(isMobile){let motionEvent=new event_TouchEvent(this,this.dataBounds,this.viewPortHandler);this.tooltipCanvasDom.ontouchstart=e=>{motionEvent.touchStart(e);};this.tooltipCanvasDom.ontouchmove=e=>{motionEvent.touchMove(e);};this.tooltipCanvasDom.ontouchend=e=>{motionEvent.touchEnd(e);};}else{let motionEvent=new event_MouseEvent(this,this.dataBounds,this.viewPortHandler);this.markEvent=new event_MarkEvent(this,this.markData,this.dataBounds,this.viewPortHandler);this.tooltipCanvasDom.onmousedown=e=>{motionEvent.mouseDown(e);this.markEvent.mouseDown(e);};this.tooltipCanvasDom.onmouseup=e=>{motionEvent.mouseUp(e);};this.tooltipCanvasDom.onmousemove=e=>{motionEvent.mouseMove(e);this.markEvent.mouseMove(e);};this.tooltipCanvasDom.onmouseleave=e=>{motionEvent.mouseLeave(e);};this.tooltipCanvasDom.onwheel=e=>{motionEvent.mouseWheel(e);};this.tooltipCanvasDom.oncontextmenu=()=>{return false;};this.markCanvasDom.onmousedown=e=>{this.markEvent.mouseDown(e);};this.markCanvasDom.onmouseup=e=>{this.markEvent.mouseUp(e);};this.markCanvasDom.onmousemove=e=>{this.markEvent.mouseMove(e);};this.markCanvasDom.onmouseleave=e=>{this.markEvent.mouseLeave(e);};this.markCanvasDom.onwheel=e=>{this.markEvent.mouseWheel(e);};this.markCanvasDom.oncontextmenu=()=>{return false;};}}catch(e){}}/**
   * 计算图表高度
   */calcChartHeight(domHeight){let xChartHeight=this.xAxis.getRequiredHeightSpace();let chartHeight=domHeight-xChartHeight;let isDisplayVolChart=this.volChart.isDisplayChart();let isDisplayIndicatorChart=this.indicatorChart.isDisplayChart();let volChartHeight=0;let indicatorChartHeight=0;if(isDisplayVolChart&&isDisplayIndicatorChart){let height=parseInt(chartHeight*0.2);volChartHeight=height;indicatorChartHeight=height;}else if(isDisplayVolChart&&!isDisplayIndicatorChart){volChartHeight=parseInt(chartHeight*0.3);}else if(!isDisplayVolChart&&isDisplayIndicatorChart){indicatorChartHeight=parseInt(chartHeight*0.3);}let candleChartHeight=chartHeight-volChartHeight-indicatorChartHeight;let contentTop=0;this.candleChart.setChartDimens(candleChartHeight,contentTop);this.markChart.setChartDimens(candleChartHeight,contentTop);contentTop+=candleChartHeight;this.volChart.setChartDimens(volChartHeight,contentTop);contentTop+=volChartHeight;this.indicatorChart.setChartDimens(indicatorChartHeight,contentTop);contentTop+=indicatorChartHeight;this.xAxisChart.setChartDimens(xChartHeight,contentTop);}/**
   * 计算不包括x轴y轴的绘制区域的尺寸
   */calcOffsets(){let offsetLeft=0;let offsetRight=0;let offsetTop=0;let offsetBottom=0;if(this.yAxis.needsOffset()){// 计算y轴最大宽度
let yAxisRequireWidthSpace=this.yAxis.getRequiredWidthSpace();if(this.yAxis.yAxisPosition===YAxisPosition.LEFT){offsetLeft+=yAxisRequireWidthSpace;}else{offsetRight+=yAxisRequireWidthSpace;}}offsetBottom+=this.xAxis.getRequiredHeightSpace();this.viewPortHandler.restrainViewPort(offsetLeft,offsetTop,offsetRight,offsetBottom);}/**
   * 设置canvas尺寸
   * @param dom
   * @param rootDomWidth
   * @param rootDomHeight
   */setCanvasDomSize(dom,rootDomWidth,rootDomHeight){dom.width=rootDomWidth*2;dom.height=rootDomHeight*2;dom.style.width=rootDomWidth+'px';dom.style.height=rootDomHeight+'px';}/**
   * 清空canvas
   * @param freshenType
   * @param rootDomWidth
   * @param rootDomHeight
   */clearCanvasRect(freshenType,rootDomWidth,rootDomHeight){// 根据刷新类型来清空画布区域
switch(freshenType){case FRESHEN_CHART:{this.chartCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);break;}case FRESHEN_TOOLTIP:{this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);break;}case FRESHEN_ALL:{this.chartCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);break;}case FRESHEN_CANDLE_CHART:{this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);this.chartCanvas.clearRect(0,this.candleChart.chartTop,rootDomWidth*2,this.candleChart.chartHeight);break;}case FRESHEN_VOL_CHART:{if(this.volChart.isDisplayChart()){this.chartCanvas.clearRect(0,this.volChart.chartTop,rootDomWidth*2,this.volChart.chartHeight);}break;}case FRESHEN_INDICATOR_CHART:{if(this.indicatorChart.isDisplayChart()){this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);this.chartCanvas.clearRect(0,this.indicatorChart.chartTop,rootDomWidth*2,this.indicatorChart.chartHeight);}break;}case FRESHEN_DRAW_LINE_CHART:{this.markCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);}}}/**
   * 刷新
   * @param freshenType 刷新类型
   */freshen(freshenType=FRESHEN_ALL){const rootDomWidth=this.rootDom.offsetWidth;const rootDomHeight=this.rootDom.offsetHeight;this.clearCanvasRect(freshenType,rootDomWidth,rootDomHeight);if(this.isShouldCalcChartHeight){this.calcChartHeight(rootDomHeight*2);this.isShouldCalcChartHeight=false;}if(this.isShouldCalcOffset){this.setCanvasDomSize(this.chartCanvasDom,rootDomWidth,rootDomHeight);this.setCanvasDomSize(this.markCanvasDom,rootDomWidth,rootDomHeight);this.setCanvasDomSize(this.tooltipCanvasDom,rootDomWidth,rootDomHeight);this.viewPortHandler.setChartDimens(rootDomWidth*2,rootDomHeight*2);this.calcOffsets();this.isShouldCalcOffset=false;}this.draw(freshenType);}/**
   * 绘制图
   */draw(freshenType){this.dataBounds.space();if(freshenType===FRESHEN_VOL_CHART){this.volChart.draw(this.chartCanvas);}else if(freshenType===FRESHEN_INDICATOR_CHART){this.indicatorChart.draw(this.chartCanvas);this.tooltipChart.draw(this.tooltipCanvas);}else if(freshenType===FRESHEN_CANDLE_CHART){this.candleChart.draw(this.chartCanvas);this.tooltipChart.draw(this.tooltipCanvas);}else if(freshenType===FRESHEN_DRAW_LINE_CHART){this.markChart.draw(this.markCanvas);}else{if(freshenType===FRESHEN_CHART||freshenType===FRESHEN_ALL){this.gridChart.draw(this.chartCanvas);this.xAxisChart.draw(this.chartCanvas);this.candleChart.draw(this.chartCanvas);this.volChart.draw(this.chartCanvas);this.indicatorChart.draw(this.chartCanvas);}if(freshenType===FRESHEN_TOOLTIP||freshenType===FRESHEN_ALL){this.tooltipChart.draw(this.tooltipCanvas);}}}/**
   * 计算指标数据
   * @param indicatorType
   * @param freshenType
   * @returns {Promise<void>}
   */async calcIndicator(indicatorType,freshenType=FRESHEN_CHART){try{switch(indicatorType){case IndicatorType.MA:{this.dataBounds.dataList=await calculationMa(this.dataBounds.dataList);break;}case IndicatorType.MACD:{this.dataBounds.dataList=await calculationMacd(this.dataBounds.dataList);break;}case IndicatorType.VOL:{this.dataBounds.dataList=await calculationVol(this.dataBounds.dataList);break;}case IndicatorType.BOLL:{this.dataBounds.dataList=await calculationBoll(this.dataBounds.dataList);break;}case IndicatorType.BIAS:{this.dataBounds.dataList=await calculationBias(this.dataBounds.dataList);break;}case IndicatorType.BRAR:{this.dataBounds.dataList=await calculationBrar(this.dataBounds.dataList);break;}case IndicatorType.CCI:{this.dataBounds.dataList=await calculationCci(this.dataBounds.dataList);break;}case IndicatorType.CR:{this.dataBounds.dataList=await calculationCr(this.dataBounds.dataList);break;}case IndicatorType.DMA:{this.dataBounds.dataList=await calculationDma(this.dataBounds.dataList);break;}case IndicatorType.DMI:{this.dataBounds.dataList=await calculationDmi(this.dataBounds.dataList);break;}case IndicatorType.KDJ:{this.dataBounds.dataList=await calculationKdj(this.dataBounds.dataList);break;}case IndicatorType.KD:{this.dataBounds.dataList=await calculationKdj(this.dataBounds.dataList);break;}case IndicatorType.RSI:{this.dataBounds.dataList=await calculationRsi(this.dataBounds.dataList);break;}case IndicatorType.PSY:{this.dataBounds.dataList=await calculationPsy(this.dataBounds.dataList);break;}case IndicatorType.TRIX:{this.dataBounds.dataList=await calculationTrix(this.dataBounds.dataList);break;}case IndicatorType.OBV:{this.dataBounds.dataList=await calculationObv(this.dataBounds.dataList);break;}case IndicatorType.VR:{this.dataBounds.dataList=await calculationVr(this.dataBounds.dataList);break;}case IndicatorType.WR:{this.dataBounds.dataList=await calculationWr(this.dataBounds.dataList);break;}case IndicatorType.MTM:{this.dataBounds.dataList=await calculationMtm(this.dataBounds.dataList);break;}case IndicatorType.EMV:{this.dataBounds.dataList=await calculationEmv(this.dataBounds.dataList);break;}case IndicatorType.SAR:{this.dataBounds.dataList=await calculationSar(this.dataBounds.dataList);break;}}this.freshen(freshenType);}catch(e){}}/**
   * 计算各图指标
   */calcChartIndicator(){let isDisplayCandleIndicator=this.candleChart.isDisplayChart();let isDisplayVol=this.volChart.isDisplayChart();let isDisplayIndicator=this.indicatorChart.isDisplayChart();if(isDisplayCandleIndicator){this.calcIndicator(this.candleChart.indicatorType);}if(isDisplayVol){this.calcIndicator(IndicatorType.VOL);}if(isDisplayIndicator){this.calcIndicator(this.indicatorChart.indicatorType);}}/**
   * 检查数据是否合法
   * @param data
   */checkData(data){if(typeof data!=='object'||data.open===null||data.open===undefined||data.close===null||data.close===undefined||data.high===null||data.high===undefined||data.low===null||data.low===undefined||data.timestamp===null||data.timestamp===undefined||data.volume===null||data.volume===undefined||data.turnover===null||data.turnover===undefined){throw new Error('The data must be object and need to contain open, close, high, low, timestamp, volume, and turnover fields');}}/**
   * 改变尺寸
   */resize(){this.isShouldCalcOffset=true;this.freshen();}/**
   * 设置配置
   * @param config
   */setConfig(config){if(config){let common=config.common;if(common){if(common.minVisibleRange>0){this.dataBounds.minRange=common.minVisibleRange;}if(common.maxVisibleRange>0&&common.maxVisibleRange>this.dataBounds.minRange){this.dataBounds.maxRange=common.maxVisibleRange;}if(common.defaultVisibleRange>0&&common.defaultVisibleRange>this.dataBounds.minRange-1&&common.defaultVisibleRange<this.dataBounds.maxRange+1){this.dataBounds.range=common.defaultVisibleRange;}}let grid=config.grid;if(grid){if(grid.display!==undefined){this.grid.display=grid.display;}if(grid.lineSize>0){this.grid.lineSize=grid.lineSize;}if(grid.lineColor){this.grid.lineColor=grid.lineColor;}}let candle=config.candle;if(candle){if(candle.chartType){this.candle.chartStyle=candle.chartType;}if(candle.timeChart){this.candle.timeChart={...this.candle.timeChart,...candle.timeChart};}if(candle.candleChart){this.candle.candleChart={...this.candle.candleChart,...candle.candleChart};}if(candle.lowestHighestPriceMarkTextColor){this.candle.lowestHighestPriceMarkTextColor=candle.lowestHighestPriceMarkTextColor;}if(candle.lowestHighestPriceMarkTextSize>0){this.candle.lowestHighestPriceMarkTextSize=candle.lowestHighestPriceMarkTextSize;}this.candle.lowestHighestValueFormatter=candle.lowestHighestValueFormatter;if(candle.highestPriceMark){this.candle.highestPriceMark={...this.candle.highestPriceMark,...candle.highestPriceMark};}if(candle.lowestPriceMark){this.candle.lowestPriceMark={...this.candle.lowestPriceMark,...candle.lowestPriceMark};}if(candle.lastPriceMark){this.candle.lastPriceMark={...this.candle.lastPriceMark,...candle.lastPriceMark};}}let indicator=config.indicator;if(indicator){if(indicator.lineSize>0){this.indicator.lineSize=indicator.lineSize;}if(indicator.increasingColor){this.indicator.increasingColor=indicator.increasingColor;}if(indicator.decreasingColor){this.indicator.decreasingColor=indicator.decreasingColor;}if(indicator.lineColors&&indicator.lineColors.length>4){this.indicator.lineColors=indicator.lineColors;}}let xAxis=config.xAxis;if(xAxis){if(xAxis.display!==null&&xAxis.display!==undefined){this.xAxis.display=xAxis.display;}if(this.xAxis.color){this.xAxis.color=xAxis.color;}if(xAxis.minHeight>=0){this.xAxis.xAxisMinHeight=xAxis.minHeight;}if(xAxis.maxHeight>=0&&xAxis.maxHeight>=this.xAxis.xAxisMinHeight){this.xAxis.xAxisMaxHeight=xAxis.maxHeight;}if(xAxis.axisLine){this.xAxis.axisLine={...this.xAxis.axisLine,...xAxis.axisLine};}if(xAxis.tickText){this.xAxis.tickText={...this.xAxis.tickText,...xAxis.tickText};}if(xAxis.tickLine){this.xAxis.tickLine={...this.xAxis.tickLine,...xAxis.tickLine};}if(xAxis.separatorLine){this.xAxis.separatorLine={...this.xAxis.separatorLine,...xAxis.separatorLine};}}let yAxis=config.yAxis;if(yAxis){if(yAxis.display!==null&&yAxis.display!==undefined){this.yAxis.display=yAxis.display;}if(yAxis.position){this.yAxis.yAxisPosition=yAxis.position;this.isShouldCalcOffset=true;}if(this.yAxis.color){this.yAxis.color=yAxis.color;}if(yAxis.minWidth>=0){this.yAxis.yAxisMinWidth=yAxis.minWidth;}if(yAxis.maxWidth>=0&&yAxis.maxWidth>=this.yAxis.yAxisMinWidth){this.yAxis.yAxisMaxWidth=yAxis.maxWidth;}if(yAxis.axisLine){this.yAxis.axisLine={...this.yAxis.axisLine,...yAxis.axisLine};}if(yAxis.tickText){this.yAxis.tickText={...this.yAxis.tickText,...yAxis.tickText};this.isShouldCalcOffset=true;}if(yAxis.tickLine){this.yAxis.tickLine={...this.yAxis.tickLine,...yAxis.tickLine};}if(yAxis.separatorLine){this.yAxis.separatorLine={...this.yAxis.separatorLine,...yAxis.separatorLine};}}let tooltip=config.tooltip;if(tooltip){if(tooltip.textSize>0){this.tooltip.textSize=tooltip.textSize;}if(tooltip.crossLine){if(tooltip.crossLine.display!==null&&tooltip.crossLine.display!==undefined){this.tooltip.crossLine.display=yAxis.display;}if(tooltip.crossLine.text){tooltip.crossLine.text={...this.tooltip.crossLine.text,...tooltip.crossLine.text};}this.tooltip.crossLine={...this.tooltip.crossLine,...tooltip.crossLine};}if(tooltip.generalData){if(tooltip.generalData.text){tooltip.generalData.text={...this.tooltip.generalData.text,...tooltip.generalData.text};}this.tooltip.generalData={...this.tooltip.generalData,...tooltip.generalData};}if(tooltip.indicatorData){if(tooltip.indicatorData.text){tooltip.indicatorData.text={...this.tooltip.indicatorData.text,...tooltip.indicatorData.text};}this.tooltip.indicatorData={...this.tooltip.indicatorData,...tooltip.indicatorData};}}let mark=config.mark;if(mark){if(mark.line){if(mark.line.highlight){mark.line.highlight={...this.mark.line.highlight,...mark.line.highlight};}this.mark.line={...this.mark.line,...mark.line};}if(mark.point){if(mark.point.highlight){mark.point.highlight={...this.mark.point.highlight,...mark.point.highlight};}this.mark.point={...this.mark.point,...mark.point};}}this.freshen();}}/**
   * 设置数据
   * @param dataList
   */setDataList(dataList){if(dataList&&Array.isArray(dataList)){for(let i=0;i<dataList.length;i++){this.checkData(dataList[i]);}this.dataBounds.dataList=dataList;this.dataBounds.moveToLast();this.calcChartIndicator();}}/**
   * 添加数据
   * @param data
   * @param index
   */addData(data,index=this.dataBounds.dataList.length){this.checkData(data);this.dataBounds.dataList[index]=data;if(this.dataBounds.min+this.dataBounds.range>=this.dataBounds.dataList.length-1){this.dataBounds.moveToLast();}this.calcChartIndicator();}/**
   * 设置主指标类型
   * @param indicatorType
   */setMainIndicatorType(indicatorType){if(this.candleChart.indicatorType!==indicatorType){this.candleChart.indicatorType=indicatorType;this.calcIndicator(indicatorType,FRESHEN_CANDLE_CHART);}}/**
   * 设置副指标类型
   * @param indicatorType
   */setSubIndicatorType(indicatorType){if(this.indicatorChart.indicatorType!==indicatorType){let shouldCalcChartHeight=this.indicatorChart.isDisplayChart()&&indicatorType===IndicatorType.NO||!this.indicatorChart.isDisplayChart()&&indicatorType!==IndicatorType.NO;this.indicatorChart.indicatorType=indicatorType;if(shouldCalcChartHeight){this.isShouldCalcChartHeight=true;}this.calcIndicator(indicatorType);this.freshen(this.isShouldCalcChartHeight?FRESHEN_ALL:FRESHEN_INDICATOR_CHART);}}/**
   * 设置是否显示vol指标
   * @param isShow Boolean
   */setShowVolIndicatorChart(isShow){if(this.volChart.isDisplayChart()!==isShow){if(isShow){this.volChart.indicatorType=IndicatorType.VOL;this.calcIndicator(IndicatorType.VOL);}else{this.volChart.indicatorType=IndicatorType.NO;}this.isShouldCalcChartHeight=true;this.freshen(FRESHEN_VOL_CHART);}}/**
   * 绘制线
   * @param markingType
   */markLine(markingType){this.markEvent.drawLineStart(markingType);}}/* harmony default export */ var chart_KLineChart = (KLineChart_KLineChart);
// CONCATENATED MODULE: ./src/klinecharts.js
const klinecharts={IndicatorType: IndicatorType,LineStyle: LineStyle,YAxisTextPosition: YAxisTextPosition,YAxisPosition: YAxisPosition,CandleStyle: CandleStyle,ChartStyle: ChartStyle,IndicatorDisplayRule: IndicatorDisplayRule,init(dom){return new chart_KLineChart(dom);},version(){return "1.1.2";}};/* harmony default export */ var src_klinecharts = __webpack_exports__["default"] = (klinecharts);

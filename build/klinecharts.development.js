...
class Grid{constructor(){this.display=false;/**
     * 边框线尺寸
     */this.lineSize=1;/**
     * 边框线颜色
...
class Event{constructor(kline,dataBounds,viewPortHandler){this.kline=kline;this.dataBounds=dataBounds;this.viewPortHandler=viewPortHandler;}/**
   ...
   */getCanvasPoint(e){let rect=this.kline.tooltipCanvasDom.getBoundingClientRect();let x=Math.round(e.clientX-rect.left);let y=Math.round(e.clientY-rect.top);return{x:x*2,y:y*2};}}/* harmony default export */ var event_Event = (Event);
...
const FRESHEN_ALL='all';const FRESHEN_TOOLTIP='tooltip';const FRESHEN_CHART='chart';const FRESHEN_CANDLE_CHART='candle_chart';const FRESHEN_VOL_CHART='vol_chart';const FRESHEN_INDICATOR_CHART='indicator_chart';const isMobile=/Android|webOS|iPhone|iPod|BlackBerry|UCBrowser/i.test(navigator.userAgent);class KLineChart_KLineChart{constructor(dom){this.rootDom=dom;this.chartCanvasDom=null;this.chartCanvas=null;this.tooltipCanvasDom=null;this.tooltipCanvas=null;this.viewPortHandler=new internal_ViewPortHandler();this.dataBounds=new internal_DataBounds(this.viewPortHandler);this.grid=new component_Grid();this.yAxis=new component_YAxis();this.xAxis=new component_XAxis();this.candle=new component_Candle();this.indicator=new component_Indicator();this.tooltip=new component_Tooltip();this.gridChart=new chart_GridChart(this.grid,this.dataBounds,this.viewPortHandler);this.candleChart=new chart_CandleChart(this.candle,this.indicator,this.yAxis,this.dataBounds,this.viewPortHandler);this.volChart=new chart_IndicatorChart(this.indicator,this.xAxis,this.yAxis,this.dataBounds,this.viewPortHandler,IndicatorType.VOL);this.indicatorChart=new chart_IndicatorChart(this.indicator,this.xAxis,this.yAxis,this.dataBounds,this.viewPortHandler);this.xAxisChart=new chart_XAxisChart(this.xAxis,this.dataBounds,this.viewPortHandler);this.tooltipChart=new chart_TooltipChart(this.tooltip,this.candle,this.indicator,this.yAxis,this.candleChart,this.volChart,this.indicatorChart,this.dataBounds,this.viewPortHandler);this.motionEvent=isMobile?new event_TouchEvent(this,this.dataBounds,this.viewPortHandler):new event_MouseEvent(this,this.dataBounds,this.viewPortHandler);// 是否需要计算整个绘图区域的尺寸
this.isShouldCalcOffset=true;// 是否需要计算图的高度
this.isShouldCalcChartHeight=true;this.init();}/**
   * 初始化
   */init(){this.rootDom.style.position='relative';this.chartCanvasDom=document.createElement('canvas');this.chartCanvasDom.style.position='absolute';this.chartCanvasDom.style.top='0';this.chartCanvasDom.style.right='0';this.chartCanvasDom.style.bottom='0';this.chartCanvasDom.style.left='0';this.tooltipCanvasDom=document.createElement('canvas');this.tooltipCanvasDom.style.position='absolute';this.tooltipCanvasDom.style.top='0';this.tooltipCanvasDom.style.right='0';this.tooltipCanvasDom.style.bottom='0';this.tooltipCanvasDom.style.left='0';try{if(isMobile){this.tooltipCanvasDom.addEventListener('touchstart',e=>{this.motionEvent.touchStart(e);},false);this.tooltipCanvasDom.addEventListener('touchmove',e=>{this.motionEvent.touchMove(e);},false);this.tooltipCanvasDom.addEventListener('touchend',e=>{this.motionEvent.touchEnd(e);},false);}else{this.tooltipCanvasDom.addEventListener('mousedown',e=>{this.motionEvent.mouseDown(e);});this.tooltipCanvasDom.addEventListener('mouseup',e=>{this.motionEvent.mouseUp(e);});this.tooltipCanvasDom.addEventListener('mousemove',e=>{this.motionEvent.mouseMove(e);});this.tooltipCanvasDom.addEventListener('mouseleave',e=>{this.motionEvent.mouseLeave(e);});// IE9, Chrome, Safari, Opera
this.tooltipCanvasDom.addEventListener('mousewheel',e=>{this.motionEvent.mouseWheel(e);},false);// Firefox
this.tooltipCanvasDom.addEventListener('DOMMouseScroll',e=>{this.motionEvent.mouseWheel(e);},false);}}catch(e){}this.rootDom.appendChild(this.chartCanvasDom);this.chartCanvas=this.chartCanvasDom.getContext('2d');this.rootDom.appendChild(this.tooltipCanvasDom);this.tooltipCanvas=this.tooltipCanvasDom.getContext('2d');this.resize();}/**
   * 计算图表高度
   */calcChartHeight(domHeight){let xChartHeight=this.xAxis.getRequiredHeightSpace();let chartHeight=domHeight-xChartHeight;let isDisplayVolChart=this.volChart.isDisplayChart();let isDisplayIndicatorChart=this.indicatorChart.isDisplayChart();let volChartHeight=0;let indicatorChartHeight=0;if(isDisplayVolChart&&isDisplayIndicatorChart){let height=chartHeight*0.2;volChartHeight=height;indicatorChartHeight=height;}else if(isDisplayVolChart&&!isDisplayIndicatorChart){volChartHeight=chartHeight*0.3;}else if(!isDisplayVolChart&&isDisplayIndicatorChart){indicatorChartHeight=chartHeight*0.3;}let candleChartHeight=chartHeight-volChartHeight-indicatorChartHeight;let contentTop=0;this.candleChart.setChartDimens(candleChartHeight,contentTop);contentTop+=candleChartHeight;this.volChart.setChartDimens(volChartHeight,contentTop);contentTop+=volChartHeight;this.indicatorChart.setChartDimens(indicatorChartHeight,contentTop);contentTop+=indicatorChartHeight;this.xAxisChart.setChartDimens(xChartHeight,contentTop);}/**
   * 计算不包括x轴y轴的绘制区域的尺寸
...
let yAxisRequireWidthSpace=this.yAxis.getRequiredWidthSpace();if(this.yAxis.yAxisPosition===YAxisPosition.LEFT){offsetLeft+=yAxisRequireWidthSpace;}else{offsetRight+=yAxisRequireWidthSpace;}}offsetBottom+=this.xAxis.getRequiredHeightSpace();this.viewPortHandler.restrainViewPort(offsetLeft,offsetTop,offsetRight,offsetBottom);}/**
   ...
   */freshen(freshenType=FRESHEN_ALL){const rootDomWidth=this.rootDom.offsetWidth;const rootDomHeight=this.rootDom.offsetHeight;// 根据刷新类型来清空画布区域
switch(freshenType){case FRESHEN_CHART:{this.chartCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);break;}case FRESHEN_TOOLTIP:{this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);break;}case FRESHEN_ALL:{this.chartCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);break;}case FRESHEN_CANDLE_CHART:{this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);this.chartCanvas.clearRect(0,this.candleChart.chartTop,rootDomWidth*2,this.candleChart.chartHeight);break;}case FRESHEN_VOL_CHART:{if(this.volChart.isDisplayChart()){this.chartCanvas.clearRect(0,this.volChart.chartTop,rootDomWidth*2,this.volChart.chartHeight);}break;}case FRESHEN_INDICATOR_CHART:{if(this.indicatorChart.isDisplayChart()){this.tooltipCanvas.clearRect(0,0,rootDomWidth*2,rootDomHeight*2);this.chartCanvas.clearRect(0,this.indicatorChart.chartTop,rootDomWidth*2,this.indicatorChart.chartHeight);}break;}}if(this.isShouldCalcChartHeight){this.calcChartHeight(rootDomHeight*2);this.isShouldCalcChartHeight=false;}if(this.isShouldCalcOffset){this.chartCanvasDom.width=rootDomWidth*2;this.chartCanvasDom.height=rootDomHeight*2;this.chartCanvasDom.style.width=rootDomWidth+'px';this.chartCanvasDom.style.height=rootDomHeight+'px';this.tooltipCanvasDom.width=rootDomWidth*2;this.tooltipCanvasDom.height=rootDomHeight*2;this.tooltipCanvasDom.style.width=rootDomWidth+'px';this.tooltipCanvasDom.style.height=rootDomHeight+'px';this.viewPortHandler.setChartDimens(rootDomWidth*2,rootDomHeight*2);this.calcOffsets();this.isShouldCalcOffset=false;}this.draw(freshenType);}/**
   * 绘制图
   */draw(freshenType){this.dataBounds.space();if(freshenType===FRESHEN_VOL_CHART){this.volChart.draw(this.chartCanvas);}else if(freshenType===FRESHEN_INDICATOR_CHART){this.indicatorChart.draw(this.chartCanvas);this.tooltipChart.draw(this.tooltipCanvas);}else if(freshenType===FRESHEN_CANDLE_CHART){this.candleChart.draw(this.chartCanvas);this.tooltipChart.draw(this.tooltipCanvas);}else{if(freshenType===FRESHEN_CHART||freshenType===FRESHEN_ALL){this.gridChart.draw(this.chartCanvas);this.xAxisChart.draw(this.chartCanvas);this.candleChart.draw(this.chartCanvas);this.volChart.draw(this.chartCanvas);this.indicatorChart.draw(this.chartCanvas);}if(freshenType===FRESHEN_TOOLTIP||freshenType===FRESHEN_ALL){this.tooltipChart.draw(this.tooltipCanvas);}}}/**
   * 计算指标数据
   * @param indicatorType
   * @param freshenType
